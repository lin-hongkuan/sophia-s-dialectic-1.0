...
```
.
├── App.tsx                  # 主应用：路由、active run 管理、history 持久化
├── components/              # 视图组件
│   ├── Arena.tsx            # 完成 / 流式结果的呈现 + Continuation Studio
...
├── services/
│   ├── sophiaService.ts     # OpenAI-compatible API 客户端 + 编排
...
4. **`components/Arena.tsx`** — 在 `modeIcon`（行 21-28）映射为合适的 lucide-react 图标。
5. **可选**：如果该 mode 需要专有渲染区域（像 `diagnosisFrame` / `thoughtExperiment` / `seminarMatrix`），在 `types.ts` 加新字段，在 `Arena.tsx` 加对应 section。
